...
class ExampleActions(UserActionsBase):
    ...
    def create_actions(self):
        ...
        self.add_device_action('ex_lock', self.device_lock_action_example)
    ...
    def device_action_example(self, action_def, _):
        ...
        if device:
            ...
            self.canonical_parent.show_message('Reset device: %s' % device.name)
    ...
    def clip_action_example(self, action_def, args):
        ...
        if clip:
            if action_def['xtrigger'] != clip:
                clip.name = args
                self.canonical_parent.show_message('Clip was renamed')
            else:
                self.canonical_parent.log_message('Error: Tried to rename X-Clip!')
                self.canonical_parent.show_message('Error: Tried to rename X-Clip!')

    def device_lock_action_example(self, action_def, _):
        """ Lock control surface script SLMK2 on selected device """
        device = action_def['device']
        if device:
            self.canonical_parent.show_message('Lock action triggered')